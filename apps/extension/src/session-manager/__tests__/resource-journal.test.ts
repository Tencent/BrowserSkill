...
import { SessionManager, type SessionContext } from "../manager";
...
describe("session resource journal", () => {
  ...
  it("recovers across worker restarts but treats a missing session epoch as stale", async () => {
    ...
    // Even though storage.local still mirrors the old epoch, it must never
    // make old Chrome ids recoverable because those ids may now belong to a
    // user's newly-created windows or tabs.
    ...
    expect(local.values.get(BROWSER_RUN_ID_KEY)).toBe("run-b");
  });
