...
import type { DraftTraceStep, StepV3, StopReason, TraceState, TraceV3 } from "@/transport/types";
...
export function applyTargetMatching(
  ...
  // A pending settle means `lastSettled` may still describe the page before
  // the previous action. If the new control cannot be found there, attaching
  // that stale state would be worse than leaving the origin unknown for the
  // reducer to repair from the observations that do exist.
  const originLooksStale =
    obs.settles.size > 0 && (!("target" in draft) || draft.target?.unmatched === true);
  if (originLooksStale) {
    rememberStepAnnotation(obs, stepId, draft);
    return;
  }
...
async function coalesceRedirectLanding(
  ...
  while (obs.pendingRedirect) {
    ...
    // `tabs.get` is asynchronous. A newer redirect hop may have arrived while
    // it was in flight; only the generation that initiated this read may
    // consume the pending landing.
    if (!obs.pendingRedirect || obs.pendingRedirect.generation !== snap.generation) {
      continue;
    }
    obs.pendingRedirect = null;
