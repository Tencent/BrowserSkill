...
关闭两项后，当前浏览器配置文件中的任务可以免确认借用标签页，并由 Agent 尽力自主完成操作。
设置自动保存，对现有会话和新会话生效。关闭借用确认会放行遵循该偏好的待确认请求；
关闭人工协助会将等待中的求助结束为 `disabled`。关闭人工协助不会关闭借用确认。
普通任务使用 `bsk session start` 遵循这些设置；“自动完成任务”本身不代表同意跳过确认。

只有用户或宿主明确要求本次同时免去已有用户标签页的借用确认、并禁用人工协助时，才使用会话覆盖：
...
技能会引导 Agent 重新观察页面并继续尝试，不能仅因求助关闭就把任务判为受阻，也不能把这个结果当作操作完成。
手机扫码、人脸验证、无法获取的短信验证码，以及纯文本模型无法识别的图形验证码可以报告受阻；
其他步骤应使用现有浏览器工具尽力处理并验证结果，只有缺少必要信息或能力、或可行方案已经尝试完毕时才报告具体阻碍。
关闭协助不增加任务授权，也不替代宿主许可。在任务授权和宿主规则允许的范围内，具备图像理解能力的模型
可以通过截图和现有交互尝试图形验证。已有的 `BSK_REQUEST_HELP=off` 使用同样的自主处理策略，不影响借用确认。
两个开关均开启、没有显式覆盖时，普通会话仍使用原有的确认和人工协助流程。

| 设置入口 | 作用范围 | 借用确认 | 人工协助 |
| --- | --- | --- | --- |
| 插件两个开关 | 当前浏览器配置 | 普通会话的默认策略 | 普通会话的默认策略 |
| `--unattended` | 单个会话 | 跳过 | 禁用 |
| `--no-confirm` | 单次借用 | 跳过 | 无影响 |
| `BSK_REQUEST_HELP=off` | 设置该变量的 CLI／Daemon 进程 | 无影响 | 禁用 |

`--unattended` 优先于插件设置；重新打开插件开关不会改变该会话，需新建普通会话才能恢复遵循插件设置。
普通会话中单次显式确认参数优先于浏览器偏好。`--no-confirm` 和环境变量禁用也需用户或宿主明确要求；
不能因为出现确认、拒绝或超时而自行启用。已明确给出的授权不必重复询问。

偏好读取失败时，后台保留已有有效值；尚无有效值时按两项开启处理，不写回默认值，也不阻止新建会话。
后续请求会重新读取，存储变更也能恢复有效策略。此时仅依赖尚未读到的关闭设置可能再次出现提示，
显式会话或进程禁用仍然有效。弹窗保留读取错误提示并禁止保存；写入失败不会被当作成功。

单次免确认使用 `bsk tab borrow <tab-id> --session <id> --no-confirm`；
`--timeout 60s` 设置借用确认的等待时间。`session start --json` 和 `session list --json`
会返回浏览器／会话的 `interaction` 策略，不包含单次借用参数和调用进程环境变量的额外覆盖。
新 CLI 参数要求 Daemon 和扩展均支持协议 1.2，旧版本会明确提示升级。
这些设置控制 BrowserSkill 自身的提示；CLI 覆盖参数不提供宿主授权校验，Agent 宿主的命令审批仍由宿主管理。
