...
describe("session.start", () => {
  ...
  it("allows noFocus with inWindow but rejects it with an existing tab", async () => {
    const { tools, calls } = setup({ "session start": START_REPLY("s1") });
    await tools.get("session.start")?.execute({ inWindow: true, noFocus: true }, makeExec());
    expect(calls[0].args).toEqual([
      "session",
      "start",
      "--request-id",
    ...
    for (const placement of [{ currentTab: true }, { tabId: 7 }]) {
      await expect(
        tools.get("session.start")?.execute({ ...placement, noFocus: true }, makeExec()),
      ).rejects.toThrow(/noFocus/);
    }
