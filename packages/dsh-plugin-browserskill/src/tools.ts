    ...
    defineTool({
      ...
      parameters: {
        ...
        noFocus: {
          type: "boolean",
          description:
            "Start without stealing focus (an inactive tab with inWindow); unavailable when reusing an existing tab.",
        },
        inWindow: {
          type: "boolean",
      ...
      async execute(args, exec) {
        ...
        const existingTab = args.currentTab === true || args.tabId !== undefined;
        if (placementCount > 1) {
          throw new Error("inWindow, currentTab, and tabId are mutually exclusive");
        }
        if (placementCount > 0 && (args.width !== undefined || args.height !== undefined)) {
          throw new Error("shared and existing-tab sessions cannot use dimensions");
        }
        if (existingTab && args.noFocus === true) {
          throw new Error("existing-tab sessions cannot use noFocus");
        }
